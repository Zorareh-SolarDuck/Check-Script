...
class CheckSummary:
    ...
    def __init__(self, filename, LocSims, model=False, Chk_Summary=True, model_save=True):
        ...
        if Chk_Summary:
            ...
            self.T2_Beam_width = self.T2_Line_Type.NormalDragLiftDiameter
            self.T1_Beam_width = self.T1_Line_Type.NormalDragLiftDiameter
    ...
    def findPlatformNum(self):
        ...
        self.MooringName = sorted(self.MooringName, key=lambda x: int(x.split('_')[1]))
    ...
    def distance(self,objName1,objName2,obj1_Node=0,obj2_Node=0):
        
        X1, Y1, Z1 = self.global_coord(objName1)[obj1_Node]
        X2, Y2, Z2 = self.global_coord(objName2)[obj2_Node]
        dist = ((X2-X1)**2+(Y2-Y1)**2+(Z2-Z1)**2)**0.5
        horz = ((X2-X1)**2+(Y2-Y1)**2)**0.5
        return X2-X1, Y2-Y1, Z2-Z1, dist, horz
    ...
    def XDir(self, string, int_pnt = 0):
        XDirMax = -100000
        XDirMin =  100000
        for i,obj in enumerate(self.Objects):
    ...
    def YDir(self, string, int_pnt = 0):
        YDirMax = -100000
        YDirMin =  100000
        for i,obj in enumerate(self.Objects):
            if obj.name.startswith(string):
    ...
    def static_global_coord(self, objName, run_static = True):
        obj = self.CheckModel[objName]
        if run_static:
            self.CheckModel.CalculateStatics()
        
        if obj.nameType == 'Line':
            parameters = ['X', 'Y', 'Z']
            run_B      = True
        ...
        if run_B:
            GXA = obj.StaticResult(parameters[0], ofx.oeEndB)
            GYA = obj.StaticResult(parameters[1], ofx.oeEndB)
            GZA = obj.StaticResult(parameters[2], ofx.oeEndB)
        else:
            GXB = None
            GYB = None
            GZB = None
    ...
    def summary_Xls(self):
        ...
        for iBeam in range(self.Num_Beam_Per_Platform):
            self.df1['OD' + str(iBeam)]                              = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x)['OD']).round(3)
            # self.df1['ID' + str(iBeam)]                              = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).ID).round(3)
            # self.df1['CGX' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).CGX).round(3)
            # self.df1['CGY' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).CGY).round(3)
            # self.df1['Mass Per Unit Length' + str(iBeam)]            = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).MLen).round(3)
            # self.df1['EIx' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmz).round(3)
            # self.df1['EIy' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmz).round(3)
            # self.df1['EA' + str(iBeam)]                              = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmz).round(3)
            # self.df1['Poisson Ratio' + str(iBeam)]                   = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmz).round(3)
            # self.df1['GJ' + str(iBeam)]                              = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmz).round(3)
            # self.df1['Tension-Torque Coupling' + str(iBeam)]         = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmz).round(3)
            # self.df1['Cdx' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmz).round(3)
            # self.df1['Cdy' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmz).round(3)
            # self.df1['Cdz' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmz).round(3)
            # self.df1['Normal Drag-Lift Dia' + str(iBeam)]            = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmz).round(3)
            # self.df1['Axial Drag-Lift Dia' + str(iBeam)]             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmz).round(3)
            # self.df1['Cax' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmx).round(3)
            # self.df1['Cay' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cay).round(3)
            # self.df1['Caz' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Caz).round(3)
            # self.df1['Cmx' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmx).round(3)
            # self.df1['Cmy' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmy).round(3)
            # self.df1['Cmz' + str(iBeam)]                             = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cmz).round(3)
            # self.df1['Cs' + str(iBeam)]                              = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Cs).round(3)
            # self.df1['Ce' + str(iBeam)]                              = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Ce).round(3)
            # self.df1['Rayleigh Damping Coefficients' + str(iBeam)]   = self.df1['Plt Num'].apply(lambda x: self.BeamProperties(x).Damp).round(3)
        ...
        data = [
            ...
            {'Item': 'X-Dir Platforms Width (c-c Float.)',  'Value': self.XDir('Floater'),                                                                          'Unit': '[m]'},
            {'Item': 'Y-Dir Platforms Width (c-c Float.)',  'Value': self.YDir('Floater'),                                                                          'Unit': '[m]'},
            ...
            {'Item': 'Platform Fairlead horz. dist',        'Value': self.distance('Con_Frld_'+self.minMoor+'_240_1','Con_Frld_'+self.minMoor+'_240_-1',0,0)[4],    'Unit': '[m]'},
            {'Item': 'X-Dir Mooring Foortprint Width',      'Value': self.XDir('M1', 1),                                                                            'Unit': '[m]'},
            {'Item': 'Y-Dir Mooring Foortprint Width',      'Value': self.YDir('M1', 1),                                                                            'Unit': '[m]'},
        ...
        self.df2['Value'] = self.df2['Value'].round(2)
        ...
        if os.path.isfile(self.ExcelFile):
            with pd.ExcelWriter(self.ExcelFile, engine='openpyxl', mode='a') as writer:
                ...
                self.df2.to_excel(writer, sheet_name=self.SheetName+'-General', index=True)
                self.df3.to_excel(writer, sheet_name=self.SheetName+'-Coupl', index=True)
                self.df4.to_excel(writer, sheet_name=self.SheetName+'-pre_T', index=True)
        else:
    ...
    def calc_pre_tension(self):
        self.df4 = pd.DataFrame(columns=['Mooring Name','Length','Pre_Tension','Initial Horiz. Dist.','Static Horiz. Dist.',
        ...
        for iName, objName in enumerate(self.MooringName):
            ...
            self.df4.loc[iName,'Mooring Name']                     = objName
            self.df4.loc[iName,'Length']                           = round(np.sum(Mooring_Object.Length),2)
            self.df4.loc[iName,'Initial Horiz. Dist.']             = round(self.distance(objName,objName,0,1)[4],2)
            self.df4.loc[iName,['End A GX','End A GY','End A GZ']] = [round(x, 2) for x in self.global_coord(objName)[0]]
            self.df4.loc[iName,['End B GX','End B GY','End B GZ']] = [round(x, 2) for x in self.global_coord(objName)[1]]
            
        self.CheckModel.CalculateStatics()
        for iName, objName in enumerate(self.MooringName): # because after static calc can't load Plar coordinates!
            Mooring_Object = self.CheckModel[objName]
            self.df4.loc[iName,'Pre_Tension'] = round(Mooring_Object.StaticResult('Effective tension', ofx.oeEndA),2)
        self.CheckModel.Reset()
