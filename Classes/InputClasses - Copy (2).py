...
class CheckSummary:
    ...
    def calc_pre_tension(self):
        self.df5 = pd.DataFrame(columns=['Mooring Name','Length','Pre_Tension','Initial Horiz. Dist.','Static Horiz. Dist.',
        ...
        for iName, objName in enumerate(self.MooringName):
            ...
            self.df5.loc[iName,'Mooring Name']                     = objName
            self.df5.loc[iName,'Length']                           = round(np.sum(Mooring_Object.Length),2)
            self.df5.loc[iName,'Initial Horiz. Dist.']             = round(self.distance(objName,objName,0,1)[4],2)
            self.df5.loc[iName,['End A GX','End A GY','End A GZ']] = [round(x, 2) for x in self.global_coord(objName)[0]]
            self.df5.loc[iName,['End B GX','End B GY','End B GZ']] = [round(x, 2) for x in self.global_coord(objName)[1]]
            
        self.CheckModel.CalculateStatics()
        for iName, objName in enumerate(self.MooringName): # because after static calc can't load Polar coordinates!
            Mooring_Object = self.CheckModel[objName]
            self.df5.loc[iName,'Pre_Tension'] = round(Mooring_Object.StaticResult('Effective tension', ofx.oeEndA),2)
            
        unit_df5 = [['[-]', '[m]', '[kN]', '[m]', '[m]', '[m]', '[m]', '[m]', '[m]', '[m]', '[m]']]
        unit_df5 = pd.DataFrame(unit_df5, columns=self.df5.columns)
        self.df5 = pd.concat([unit_df5, self.df5]).reset_index(drop=True)            
